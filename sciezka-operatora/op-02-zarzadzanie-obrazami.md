...
### 2.1.1. Wewnętrzny Mechanizm Rejestru: Analiza Usługi `image-registry.openshift-image-registry.svc:5000`
...
### 2.1.2. Wystawianie Rejestru na Zewnątrz: Konfiguracja `Route` i Względy Bezpieczeństwa
...
### 2.1.3. Uwierzytelnianie i Dostęp: Proces Logowania `podman login` z Użyciem Tokenów `oc`
...
### 2.2.1. Geneza i Cel `ImageStream`: Rozwiązanie Problemów Czystego Kubernetes
...
### 2.2.2. Dogłębna Analiza Obiektów: Różnice i Współdziałanie `ImageStream` (IS) vs. `ImageStreamTag` (IST)
...
### 2.2.3. Śledzenie Rejestrów Zewnętrznych: Konfiguracja Importu i Synchronizacji (np. z Docker Hub)
...
### 2.2.4. Rola `ImageChangeTrigger` (Zajawka)
...
### 2.3.1. Anatomia Obiektu `BuildConfig` (BC)
...
### 2.3.2. Triggery Budowania (Część 1): Webhooki `GitHub` i `Generic`
...
### 2.3.3. Triggery Budowania (Część 2): Reaktywne Potoki z `ImageChange` i `ConfigChange`
...
### 2.4.1. Source-to-Image (S2I): Dogłębna Analiza
...
### 2.4.2. Docker: Strategia Oparta na `Dockerfile`
...
### 2.4.3. Pipeline: Wprowadzenie do Złożonych Przepływów CI/CD
...
### 2.4.4. Rekomendacje: Wybór Odpowiedniej Strategii
...
### 2.5.1. Inicjalizacja Aplikacji: Analiza Polecenia `oc new-app` i Tworzone Obiekty
...
### 2.5.2. Monitorowanie Procesu Budowania: Efektywne Użycie `oc logs -f bc/my-app`
...
### 2.5.3. Wystawianie Aplikacji: Kreacja `Route` za Pomocą `oc expose svc/my-app`
...
### 2.5.4. Weryfikacja End-to-End: Od `git push` do Działającej Usługi
