...
### $oc get events -w --all-namespaces$: Globalny Puls Klastra
...
### Dekodowanie Strumienia Zdarzeń: Interpretacja Kluczowych Błędów
...
#### Dogłębna analiza $FailedScheduling$
...
#### Dogłębna analiza $ImagePullBackOff$ / $ErrImagePull$
...
### Studium Przypadku: Dlaczego mój Pod jest w stanie $Pending$?
...
#### Scenariusz 1: Brak zasobów (CPU/RAM)
...
#### Scenariusz 2: Tainty i Tolerancje
...
#### Scenariusz 3: Niezwiązane Wolumeny (PersistentVolumeClaims)
...
#### Scenariusz 4: Konflikty Powinowactwa (Affinity) / Antypowinowactwa
...
### Różnica między $oc logs -f$ a $oc logs -p$
...
### Studium Przypadku: Rozwiązywanie problemu $CrashLoopBackOff$
...
### Krytyczne Ograniczenie: Tylko dla Działających Kontenerów
...
### Checklista Diagnostyki Wewnątrz Kontenera
...
#### 3. Testowanie Łączności Sieciowej
...
#### Typowe błędy i ich debugowanie
...
### Debugowanie Procesów $DeploymentConfig$ (DC)
...
#### Metodologia debugowania $DeploymentConfig$
...
### Potęga $oc debug pod/...$: Narzędzie do $CrashLoopBackOff$
...
**Jak to działa:** Kluczową rzeczą do zrozumienia jest to, że $oc debug$ **nie** łączy się magicznie z crashującym Podem – to niemożliwe. Zamiast tego, wykonuje znacznie sprytniejszą operację:
...
#### Praktyczny przepływ pracy z $oc debug$:
...
### Tabela 3: Porównanie kluczowych poleceń diagnostycznych Poda
...
### Wprowadzenie do poleceń $oc adm$ (Administrator)
...
### Tabela 4: Ostateczna Checklista Rozwiązywania Problemów z Podem

#### Scenariusz 1: $STATUS: Pending$
...
#### Scenariusz 2: $STATUS: ImagePullBackOff$ / $ErrImagePull$
...
#### Scenariusz 3: $STATUS: CreateContainerConfigError$
...
#### Scenariusz 4: $STATUS: CrashLoopBackOff$
...
#### Scenariusz 5: $STATUS: Running$, $READY: 0/1$
